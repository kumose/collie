...
struct StringHash : collie::Hash<std::string_view> {
...
TEST(Table, Prefetch) {
  ...
  const auto now = [] { return collie::base_internal::CycleClock::Now(); };
...
TEST(Table, NoThrowMoveConstruct) {
  ASSERT_TRUE(
      std::is_nothrow_copy_constructible<collie::Hash<std::string_view>>::value);
...
TEST(Table, NoThrowMoveAssign) {
  ASSERT_TRUE(
      std::is_nothrow_move_assignable<collie::Hash<std::string_view>>::value);
  ASSERT_TRUE(
  ...
  ASSERT_TRUE(
      collie::allocator_traits<std::allocator<int>>::is_always_equal::value);
  EXPECT_TRUE(std::is_nothrow_move_assignable<StringTable>::value);
}

TEST(Table, NoThrowSwappable) {
  ASSERT_TRUE(
      priv::IsNoThrowSwappable<collie::Hash<std::string_view>>());
...
struct VerifyResultOf<C, Table, collie::void_t<C<Table>>> : std::true_type {};

#if PHMAP_HAVE_STD_STRING_VIEW
TEST(Table, HeterogeneousLookupOverloads) {
  using NonTransparentTable =
      raw_hash_set<StringPolicy, collie::Hash<std::string_view>,
  ...
  using TransparentTable = raw_hash_set<
      StringPolicy,
      collie::priv::hash_default_hash<std::string_view>,
      collie::priv::hash_default_eq<std::string_view>,
      std::allocator<int>>;
