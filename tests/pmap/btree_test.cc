...
namespace {
    ...
    using ::collie::test_internal::CopyableMovableInstance;
    using ::collie::test_internal::InstanceTracker;
    using ::collie::test_internal::MovableOnlyInstance;
...
namespace {
    ...
    void SetTest() {
        using BtreeSet = collie::btree_set<K>;
        using CountingBtreeSet =
            collie::btree_set<K, std::less<K>, PropagatingCountingAlloc<K>>;
    ...
    template <typename K, int N = 256>
    void MapTest() {
        using BtreeMap = collie::btree_map<K, K>;
        using CountingBtreeMap =
            collie::btree_map<K, K, std::less<K>,
    ...
    template <typename K, int N = 256>
    void MultiSetTest() {
        using BtreeMSet = collie::btree_multiset<K>;
        using CountingBtreeMSet =
            collie::btree_multiset<K, std::less<K>, PropagatingCountingAlloc<K>>;
    ...
    template <typename K, int N = 256>
    void MultiMapTest() {
        using BtreeMMap = collie::btree_multimap<K, K>;
        using CountingBtreeMMap =
            collie::btree_multimap<K, K, std::less<K>,
                                  PropagatingCountingAlloc<std::pair<const K, K>>>;
    ...
    TEST(Btree, NoHeterogeneousLookupWithoutAlias) {
        using StringSet = collie::btree_set<std::string, NonTransparentCompare>;
        StringSet s;
        ...
        using StringMultiSet =
            collie::btree_multiset<std::string, NonTransparentCompare>;
        StringMultiSet ms;
        ms.insert("hello");
        ms.insert("world");
        ms.insert("world");
    ...
    TEST(Btree, HeterogeneousLookupDoesntDegradePerformance) {
        using StringSet = collie::btree_set<StringLike>;
        StringSet s;
        for (int i = 0; i < 100; ++i) {
    ...
    TEST(Btree, SwapKeyCompare) {
        using SubstringSet = collie::btree_set<std::string, SubstringLess>;
    ...
    TEST(Btree, Comparison) {
        ...
        collie::btree_set<int64_t> my_set;
        for (int i = 0; i < kSetSize; ++i) {
            my_set.insert(i);
        }
        collie::btree_set<int64_t> my_set_copy(my_set);
        ...
        collie::btree_map<std::string, int64_t> my_map;
        for (int i = 0; i < kSetSize; ++i) {
            my_map[std::string(i, 'a')] = i;
        }
        collie::btree_map<std::string, int64_t> my_map_copy(my_map);
    ...
    TEST(Btree, RangeCtorSanity) {
        ...
        collie::btree_multiset<int> tmset(ivec.begin(), ivec.end());
        collie::btree_multimap<int, int> tmmap(imap.begin(), imap.end());
        collie::btree_set<int> tset(ivec.begin(), ivec.end());
        collie::btree_map<int, int> tmap(imap.begin(), imap.end());
    ...
    TEST(Btree, BtreeMapCanHoldMoveOnlyTypes) {
        collie::btree_map<std::string, std::unique_ptr<std::string>> m;
    ...
    TEST(Btree, InitializerListConstructor) {
        collie::btree_set<std::string> set({"a", "b"});
        EXPECT_EQ(set.count("a"), 1);
        EXPECT_EQ(set.count("b"), 1);

        collie::btree_multiset<int> mset({1, 1, 4});
        EXPECT_EQ(mset.count(1), 2);
        EXPECT_EQ(mset.count(4), 1);

        collie::btree_map<int, int> map({{1, 5}, {2, 10}});
        EXPECT_EQ(map[1], 5);
        EXPECT_EQ(map[2], 10);

        collie::btree_multimap<int, int> mmap({{1, 5}, {1, 10}});
    ...
    TEST(Btree, InitializerListInsert) {
        collie::btree_set<std::string> set;
        set.insert({"a", "b"});
        EXPECT_EQ(set.count("a"), 1);
        EXPECT_EQ(set.count("b"), 1);

        collie::btree_multiset<int> mset;
        mset.insert({1, 1, 4});
        EXPECT_EQ(mset.count(1), 2);
        EXPECT_EQ(mset.count(4), 1);

        collie::btree_map<int, int> map;
        map.insert({{1, 5}, {2, 10}});
        ...
        collie::btree_multimap<int, int> mmap;
    ...
    void AssertKeyCompareToAdapted() {
        ...
        static_assert(
            std::is_same<collie::weak_ordering,
            collie::invoke_result_t<Adapted, const K &, const K &>>::value,
    ...
    void AssertKeyCompareToNotAdapted() {
        ...
        static_assert(
            std::is_same<bool,
            collie::invoke_result_t<Unadapted, const K &, const K &>>::value,
    ...
    TEST(Btree, RValueInsert) {
        ...
        collie::btree_set<MovableOnlyInstance> set;
        ...
        collie::btree_multiset<MovableOnlyInstance> mset;
        ...
        collie::btree_map<int, MovableOnlyInstance> map;
        ...
        collie::btree_multimap<int, MovableOnlyInstance> mmap;
...
namespace {
    ...
    TEST(Btree, MovesComparisonsCopiesSwapsTracking) {
        ...
        PHMAP_IF_CONSTEXPR (sizeof(void *) == 8) {
            EXPECT_EQ(BtreeNodePeer::GetNumValuesPerNode<collie::btree_set<int32_t>>(),
    ...
    struct MovableOnlyInstanceThreeWayCompare {
        collie::weak_ordering operator()(const MovableOnlyInstance &a,
    ...
    TEST(Btree, BtreeMapCanHoldNoDefaultCtorTypes) {
        collie::btree_map<NoDefaultCtor, NoDefaultCtor> m;
    ...
    TEST(Btree, BtreeMultimapCanHoldNoDefaultCtorTypes) {
        collie::btree_multimap<NoDefaultCtor, NoDefaultCtor> m;
    ...
    TEST(Btree, BtreeMultimapEmplace) {
        ...
        const char value0[] = "a";
        collie::btree_multimap<int, std::string> s;
    ...
    TEST(Btree, StrSplitCompatible) {
        const collie::btree_set<std::string> split_set = collie::StrSplit("a,b,c", ',');
        const collie::btree_set<std::string> expected_set = {"a", "b", "c"};

        EXPECT_EQ(split_set, expected_set);
    }
#endif

    // We can't use EXPECT_EQ/etc. to compare collie::weak_ordering because they
    // convert literal 0 to int and collie::weak_ordering can only be compared with
    ...
    TEST(Btree, ValueComp) {
        ...
        collie::btree_map<std::string, int> m2;
    ...
    TEST(Btree, DefaultConstruction) {
        collie::btree_set<int> s;
        collie::btree_map<int, int> m;
        collie::btree_multiset<int> ms;
        collie::btree_multimap<int, int> mm;
    ...
    TEST(Btree, SwissTableHashable) {
        ...
        using set = collie::btree_set<int>;
        EXPECT_TRUE(collie::VerifyTypeImplementsPhmapHashCorrectly({
                    set{},
                        set{1},
                            set{2},
                                set{1, 2},
        ...
        using mset = collie::btree_multiset<int>;
        EXPECT_TRUE(collie::VerifyTypeImplementsPhmapHashCorrectly({
                    mset{},
                        mset{1},
                            mset{1, 1},
        ...
        using map = collie::btree_map<int, int>;
        EXPECT_TRUE(collie::VerifyTypeImplementsPhmapHashCorrectly({
                    map{},
                        map{{1, 0}},
                            map{{1, 1}},
        ...
        using mmap = collie::btree_multimap<int, int>;
        EXPECT_TRUE(collie::VerifyTypeImplementsPhmapHashCorrectly({
                    mmap{},
                        mmap{{1, 0}},
                            mmap{{1, 1}},
    ...
    TEST(Btree, ComparableSet) {
        collie::btree_set<int> s1 = {1, 2};
        collie::btree_set<int> s2 = {2, 3};
        EXPECT_LT(s1, s2);
        EXPECT_LE(s1, s2);
        EXPECT_LE(s1, s1);
        EXPECT_GT(s2, s1);
    ...
    TEST(Btree, ComparableMultiset) {
        collie::btree_multiset<int> s1 = {1, 2};
        collie::btree_multiset<int> s2 = {2, 3};
        EXPECT_LT(s1, s2);
        EXPECT_LE(s1, s2);
        EXPECT_LE(s1, s1);
        EXPECT_GT(s2, s1);
    ...
    TEST(Btree, ComparableMap) {
        collie::btree_map<int, int> s1 = {{1, 2}};
        collie::btree_map<int, int> s2 = {{2, 3}};
        EXPECT_LT(s1, s2);
        EXPECT_LE(s1, s2);
        EXPECT_LE(s1, s1);
        EXPECT_GT(s2, s1);
    ...
    TEST(Btree, ComparableMultimap) {
        collie::btree_multimap<int, int> s1 = {{1, 2}};
        collie::btree_multimap<int, int> s2 = {{2, 3}};
        EXPECT_LT(s1, s2);
        EXPECT_LE(s1, s2);
        EXPECT_LE(s1, s1);
        EXPECT_GT(s2, s1);
    ...
    TEST(Btree, ComparableSetWithCustomComparator) {
        ...
        collie::btree_set<int, std::greater<int>> s1 = {1, 2};
        collie::btree_set<int, std::greater<int>> s2 = {2, 3};
        EXPECT_LT(s1, s2);
        EXPECT_LE(s1, s2);
        EXPECT_LE(s1, s1);
        EXPECT_GT(s2, s1);
    ...
    TEST(Btree, EraseReturnsIterator) {
        collie::btree_set<int> set = {1, 2, 3, 4, 5};
    ...
    TEST(Btree, ExtractTracking) {
        TestExtractWithTrackingForSet<collie::btree_set<MovableOnlyInstance>>();
        TestExtractWithTrackingForSet<collie::btree_multiset<MovableOnlyInstance>>();
        TestExtractWithTrackingForMap<
            collie::btree_map<CopyableMovableInstance, MovableOnlyInstance>>();
        TestExtractWithTrackingForMap<
            collie::btree_multimap<CopyableMovableInstance, MovableOnlyInstance>>();
    }

    TEST(Btree, ExtractAndInsertNodeHandleMultiSet) {
        collie::btree_multiset<int> src1 = {1, 2, 3, 3, 4, 5};
        auto nh = src1.extract(src1.find(3));
        EXPECT_THAT(src1, ElementsAre(1, 2, 3, 4, 5));
        collie::btree_multiset<int> other;
        auto res = other.insert(std::move(nh));
        EXPECT_THAT(other, ElementsAre(3));
        EXPECT_EQ(res, other.find(3));

        collie::btree_multiset<int> src2 = {3, 4};
        nh = src2.extract(src2.find(3));
        EXPECT_THAT(src2, ElementsAre(4));
        res = other.insert(std::move(nh));
    ...
    TEST(Btree, ExtractAndInsertNodeHandleMap) {
        collie::btree_map<int, int> src1 = {{1, 2}, {3, 4}, {5, 6}};
        auto nh = src1.extract(src1.find(3));
        EXPECT_THAT(src1, ElementsAre(Pair(1, 2), Pair(5, 6)));
        collie::btree_map<int, int> other;
        collie::btree_map<int, int>::insert_return_type res =
        ...
        collie::btree_map<int, int> src2 = {{3, 6}};
        nh = src2.extract(src2.find(3));
    ...
    TEST(Btree, ExtractAndInsertNodeHandleMultiMap) {
        collie::btree_multimap<int, int> src1 = {{1, 2}, {3, 4}, {5, 6}};
        auto nh = src1.extract(src1.find(3));
        EXPECT_THAT(src1, ElementsAre(Pair(1, 2), Pair(5, 6)));
        collie::btree_multimap<int, int> other;
        auto res = other.insert(std::move(nh));
        EXPECT_THAT(other, ElementsAre(Pair(3, 4)));
        EXPECT_EQ(res, other.find(3));

        collie::btree_multimap<int, int> src2 = {{3, 6}};
        nh = src2.extract(src2.find(3));
        EXPECT_TRUE(src2.empty());
        res = other.insert(std::move(nh));
    ...
    TEST(Btree, InsertHintNodeHandle) {
        ...
        {
            collie::btree_set<int> src = {1, 2, 3, 4, 5};
            auto nh = src.extract(src.find(3));
            EXPECT_THAT(src, ElementsAre(1, 2, 4, 5));
            collie::btree_set<int> other = {0, 100};
            // Test a correct hint.
        ...
        collie::btree_multiset<InsertMultiHintData, InsertMultiHintDataKeyCompare> src =
            {{1, 2}, {3, 4}, {3, 5}};
        auto nh = src.extract(src.lower_bound(3));
        EXPECT_EQ(nh.value(), (InsertMultiHintData{3, 4}));
        collie::btree_multiset<InsertMultiHintData, InsertMultiHintDataKeyCompare>
            other = {{3, 1}, {3, 2}, {3, 3}};
    ...
    struct IntCompareToCmp {
        collie::weak_ordering operator()(int a, int b) const {
            if (a < b) return collie::weak_ordering::less;
            if (a > b) return collie::weak_ordering::greater;
            return collie::weak_ordering::equivalent;
        }
    };

    TEST(Btree, MergeIntoUniqueContainers) {
        collie::btree_set<int, IntCompareToCmp> src1 = {1, 2, 3};
        collie::btree_multiset<int> src2 = {3, 4, 4, 5};
        collie::btree_set<int> dst;
    ...
    TEST(Btree, MergeIntoUniqueContainersWithCompareTo) {
        collie::btree_set<int, IntCompareToCmp> src1 = {1, 2, 3};
        collie::btree_multiset<int> src2 = {3, 4, 4, 5};
        collie::btree_set<int, IntCompareToCmp> dst;
    ...
    TEST(Btree, MergeIntoMultiContainersWithCompareTo) {
        collie::btree_set<int, IntCompareToCmp> src1 = {1, 2, 3};
        collie::btree_multiset<int> src2 = {3, 4, 4, 5};
        collie::btree_multiset<int, IntCompareToCmp> dst;
    ...
    TEST(Btree, MergeIntoMultiMapsWithDifferentComparators) {
        collie::btree_map<int, int, IntCompareToCmp> src1 = {{1, 1}, {2, 2}, {3, 3}};
        collie::btree_multimap<int, int, std::greater<int>> src2 = {
            {5, 5}, {4, 1}, {4, 4}, {3, 2}};
        collie::btree_multimap<int, int> dst;
    ...
    struct KeyCompareToWeakOrdering {
        template <typename T>
        collie::weak_ordering operator()(const T &a, const T &b) const {
            return a < b ? collie::weak_ordering::less
                       : a == b ? collie::weak_ordering::equivalent
                       : collie::weak_ordering::greater;
        }
    };

    struct KeyCompareToStrongOrdering {
        template <typename T>
        collie::strong_ordering operator()(const T &a, const T &b) const {
            return a < b ? collie::strong_ordering::less
                       : a == b ? collie::strong_ordering::equal
                       : collie::strong_ordering::greater;
        }
    };

    TEST(Btree, UserProvidedKeyCompareToComparators) {
        collie::btree_set<int, KeyCompareToWeakOrdering> weak_set = {1, 2, 3};
        EXPECT_TRUE(weak_set.contains(2));
        EXPECT_FALSE(weak_set.contains(4));

        collie::btree_set<int, KeyCompareToStrongOrdering> strong_set = {1, 2, 3};
        EXPECT_TRUE(strong_set.contains(2));
        EXPECT_FALSE(strong_set.contains(4));
    }

    TEST(Btree, TryEmplaceBasicTest) {
        collie::btree_map<int, std::string> m;
    ...
    TEST(Btree, TryEmplaceWithHintWorks) {
        ...
        collie::btree_map<int, int, Cmp> m(cmp);
        for (int i = 0; i < 128; ++i) {
            m.emplace(i, i);
        }
    ...
    TEST(Btree, TryEmplaceWithBadHint) {
        collie::btree_map<int, int> m = {{1, 1}, {9, 9}};
    ...
    TEST(Btree, MoveAssignmentAllocatorPropagation) {
        ...
        {
            collie::btree_set<MovableOnlyInstance, std::less<MovableOnlyInstance>,
