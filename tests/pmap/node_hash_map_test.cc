...
using MapTypes = ::testing::Types<
    collie::THIS_HASH_MAP<int, int, StatefulTestingHash, StatefulTestingEqual,
                        Alloc<std::pair<const int, int>>>,
    collie::THIS_HASH_MAP<std::string, std::string, StatefulTestingHash,
...
using M = collie::THIS_HASH_MAP<std::string, Tracked<int>>;
...
TEST(THIS_TEST_NAME, AssignRecursive) {
  struct Tree {
    ...
    collie::THIS_HASH_MAP<int, Tree> children;
  };
  Tree root;
...
TEST(FlatHashMap, MoveOnlyKey) {
  ...
  collie::THIS_HASH_MAP<Key, int, Hash, Eq> m;
  m[Key()];
}
...
TEST(THIS_TEST_NAME, MergeExtractInsert) {
  collie::THIS_HASH_MAP<NonMovableKey, int, NonMovableKeyHash, NonMovableKeyEq>
      set1, set2;
