...
namespace test_internal {
    ...
    public:
        ...
        collie::weak_ordering compare(const BaseCountedInstance& x) const {
            ++num_comparisons_;
            return value_ < x.value_
                            ? collie::weak_ordering::less
                            : value_ == x.value_ ? collie::weak_ordering::equivalent
...
namespace priv {
...
#if 0
    template <>
    struct Generator<collie::Time> {
        int maxval;
        explicit Generator(int m) : maxval(m) {}
        collie::Time operator()(int i) const { return collie::FromUnixMillis(i); }
    };
#endif
    ...
    inline std::vector<int> GenerateNumbersWithSeed(size_t n, int maxval, int seed) {
        ...
        collie::flat_hash_set<int> unique_values;
        if (values.size() < n) {
