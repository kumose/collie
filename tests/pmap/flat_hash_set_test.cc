...
using ::collie::priv::hash_internal::Enum;
using ::collie::priv::hash_internal::EnumClass;
...
template <class T>
using Set =
    collie::THIS_HASH_SET<T, StatefulTestingHash, StatefulTestingEqual, Alloc<T>>;
...
TEST(THIS_TEST_NAME, EmplaceString) {
  ...
  collie::THIS_HASH_SET<std::string_view> hs(v.begin(), v.end());
...
TEST(THIS_TEST_NAME, MergeExtractInsert) {
  ...
  collie::THIS_HASH_SET<std::unique_ptr<int>, Hash, Eq> set1, set2;
  set1.insert(collie::make_unique<int>(7));
  set1.insert(collie::make_unique<int>(17));

  set2.insert(collie::make_unique<int>(7));
  set2.insert(collie::make_unique<int>(19));
  ...
  auto node = set1.extract(collie::make_unique<int>(7));
  EXPECT_TRUE(node);
  ...
  node = set1.extract(collie::make_unique<int>(17));
  EXPECT_TRUE(node);
  EXPECT_THAT(node.value(), Pointee(17));
  EXPECT_THAT(set1, UnorderedElementsAre(Pointee(19)));

  node.value() = collie::make_unique<int>(23);
