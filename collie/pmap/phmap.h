...
void ConstructFromTupleImpl(Alloc* alloc, T* ptr, Tuple&& t,
                            collie::index_sequence<I...>) {
    collie::allocator_traits<Alloc>::construct(
...
decltype(std::declval<F>()(std::declval<T>())) WithConstructedImpl(
    Tuple&& t, collie::index_sequence<Is...>, F&& f) {
    return WithConstructedImplF<T, F>{std::forward<F>(f)}(
        std::get<Is>(std::forward<Tuple>(t))...);
}

template <class T, size_t... Is>
auto TupleRefImpl(T&& t, collie::index_sequence<Is...>)
...
public:
    ...
    using pointer = typename collie::allocator_traits<
        allocator_type>::template rebind_traits<value_type>::pointer;
    using const_pointer = typename collie::allocator_traits<
...
private:
    ...
    using Layout = collie::priv::Layout<ctrl_t, slot_type>;
    ...
    using AllocTraits = collie::allocator_traits<allocator_type>;
    using SlotAlloc = typename collie::allocator_traits<
        allocator_type>::template rebind_alloc<slot_type>;
    using SlotAllocTraits = typename collie::allocator_traits<
    ...
    using RequiresInsertable = typename std::enable_if<
        collie::disjunction<std::is_convertible<T, init_type>,
...
public:
    ...
    public:
        ...
        using reference =
            collie::conditional_t<PolicyTraits::constant_iterators::value,
                                 const value_type&, value_type&>;
        using pointer = collie::remove_reference_t<reference>*;
    ...
    //   // copies the strings into the set.
    //   collie::flat_hash_set<std::string> s = {"abc", "def"};
    //
    ...
    //
    //   collie::flat_hash_set<int> a, b{a};
    //
    ...
    raw_hash_set(raw_hash_set&& that) noexcept(
        ...
        : ctrl_(collie::exchange(that.ctrl_, EmptyGroup())),
        slots_(collie::exchange(that.slots_, nullptr)),
        size_(collie::exchange(that.size_, 0)),
        capacity_(collie::exchange(that.capacity_, 0)),
        infoz_(collie::exchange(that.infoz_, HashtablezInfoHandle())),
    ...
    raw_hash_set& operator=(raw_hash_set&& that) noexcept(
        collie::allocator_traits<allocator_type>::is_always_equal::value&&
    ...
    template <class InputIt, typename collie::enable_if_t<has_difference_operator<InputIt>::value, int> = 0>
    ...
    template <class... Args, typename std::enable_if<!IsDecomposable<Args...>::value, int>::type = 0>
    std::pair<iterator, bool> emplace(Args&&... args) {
        typename collie::aligned_storage<sizeof(slot_type), alignof(slot_type)>::type
            raw;
    ...
    template <class... Args, typename std::enable_if<!IsDecomposable<Args...>::value, int>::type = 0>
    std::pair<iterator, bool> emplace_with_hash(size_t hashval, Args&&... args) {
        typename collie::aligned_storage<sizeof(slot_type), alignof(slot_type)>::type raw;
        slot_type* slot = reinterpret_cast<slot_type*>(&raw);
...
private:
    ...
    friend struct collie::priv::hashtable_debug_internal::HashtableDebugAccess;
...
public:
    ...
    MappedReference<P> at(const key_arg<K>& key) {
        ...
        if (it == this->end()) 
            collie::base_internal::ThrowStdOutOfRange("phmap at(): lookup non-existent key");
...
protected:
    using Lockable      = collie::LockableImpl<Mtx_>;
...
private:
    ...
    template <class T>
    using RequiresInsertable = typename std::enable_if<
        collie::disjunction<std::is_convertible<T, init_type>, SameAsElementReference<T>>::value, int>::type;
...
public:
    ...
    explicit parallel_hash_set(size_t bucket_cnt, 
        ...
        parallel_hash_set(typename Inner::Params{bucket_cnt, hash_param, eq, alloc}, 
                          collie::make_index_sequence<num_tables>{})
    {}

    template <std::size_t... i>
    parallel_hash_set(typename Inner::Params const &p,
                      collie::index_sequence<i...>) : sets_{((void)i, p)...}
    {}
#else
    explicit parallel_hash_set(size_t bucket_cnt, 
                               const hasher& hash_param    = hasher(),
    ...
    bool if_contains_unsafe(const key_arg<K>& key, F&& f) const {
        return const_cast<parallel_hash_set*>(this)->template 
            modify_if_impl<K, F, LockableBaseImpl<collie::NullMutex>::DoNothing>(key, std::forward<F>(f));
    }
    ...
    {
        using std::swap;
        using Lockable2 = collie::LockableImpl<Mtx2_>;
...
struct FlatHashSetPolicy 
{
    ...
    static void construct(Allocator* alloc, slot_type* slot, Args&&... args) {
        collie::allocator_traits<Allocator>::construct(*alloc, slot,
                                                      std::forward<Args>(args)...);
    }

    template <class Allocator>
    static void destroy(Allocator* alloc, slot_type* slot) {
        collie::allocator_traits<Allocator>::destroy(*alloc, slot);
    }
    ...
    template <class F, class... Args>
    static decltype(collie::priv::DecomposeValue(
                        std::declval<F>(), std::declval<Args>()...))
    apply(F&& f, Args&&... args) {
        return collie::priv::DecomposeValue(
...
struct FlatHashMapPolicy 
{
    ...
    template <class F, class... Args>
    static decltype(collie::priv::DecomposePair(
                        std::declval<F>(), std::declval<Args>()...))
    apply(F&& f, Args&&... args) {
        return collie::priv::DecomposePair(std::forward<F>(f),
                                                        std::forward<Args>(args)...);
    }
...
struct NodeHashSetPolicy
    : collie::priv::node_hash_policy<T&, NodeHashSetPolicy<T>>
{
    using key_type = T;
    using init_type = T;
    using constant_iterators = std::true_type;
    ...
    template <class Allocator, class... Args>
        ...
        using ValueAlloc =
            typename collie::allocator_traits<Allocator>::template rebind_alloc<T>;
        ValueAlloc value_alloc(*alloc);
        T* res = collie::allocator_traits<ValueAlloc>::allocate(value_alloc, 1);
        collie::allocator_traits<ValueAlloc>::construct(value_alloc, res,
                                                       std::forward<Args>(args)...);
        return res;
    }
    ...
    template <class Allocator>
        ...
        using ValueAlloc =
            typename collie::allocator_traits<Allocator>::template rebind_alloc<T>;
        ValueAlloc value_alloc(*alloc);
        collie::allocator_traits<ValueAlloc>::destroy(value_alloc, elem);
        collie::allocator_traits<ValueAlloc>::deallocate(value_alloc, elem, 1);
    }
...
class NodeHashMapPolicy
    : public collie::priv::node_hash_policy<
...
public:
    ...
    template <class Allocator, class... Args>
        ...
        using PairAlloc = typename collie::allocator_traits<
            Allocator>::template rebind_alloc<value_type>;
        PairAlloc pair_alloc(*alloc);
        value_type* res =
            collie::allocator_traits<PairAlloc>::allocate(pair_alloc, 1);
        collie::allocator_traits<PairAlloc>::construct(pair_alloc, res,
                                                      std::forward<Args>(args)...);
        return res;
    }
    ...
    template <class Allocator>
        ...
        using PairAlloc = typename collie::allocator_traits<
            Allocator>::template rebind_alloc<value_type>;
        PairAlloc pair_alloc(*alloc);
        collie::allocator_traits<PairAlloc>::destroy(pair_alloc, pair);
        collie::allocator_traits<PairAlloc>::deallocate(pair_alloc, pair, 1);
    }
...
struct HashEq<T*> 
{
    struct Hash {
        ...
        size_t operator()(const U& ptr) const {
            // we want collie::Hash<T*> and not collie::Hash<const T*>
            // so "struct std::hash<T*> " override works
            return collie::Hash<T*>{}((T*)(uintptr_t)HashEq::ToPtr(ptr));
        }
    };
...
{};
template<typename T>
struct has_member_type_raw_hash_set<T, collie::void_t<typename T::raw_hash_set>> : std::true_type
{};
...
{};
template<typename T>
struct has_member_type_EmbeddedSet<T, collie::void_t<typename T::EmbeddedSet>> : std::true_type
{};
...
// -----------------------------------------------------------------------------
// collie::flat_hash_set
// -----------------------------------------------------------------------------
// An `collie::flat_hash_set<T>` is an unordered associative container which has
...
class flat_hash_set
    : public collie::priv::raw_hash_set<
          collie::priv::FlatHashSetPolicy<T>, Hash, Eq, Alloc>
{
...
// -----------------------------------------------------------------------------
// collie::flat_hash_map
// -----------------------------------------------------------------------------
//
// An `collie::flat_hash_map<K, V>` is an unordered associative container which
...
class flat_hash_map : public collie::priv::raw_hash_map<
                          collie::priv::FlatHashMapPolicy<K, V>,
                          Hash, Eq, Alloc> {
...
// -----------------------------------------------------------------------------
// collie::node_hash_set
// -----------------------------------------------------------------------------
// An `collie::node_hash_set<T>` is an unordered associative container which
// has been optimized for both speed and memory footprint in most common use
...
class node_hash_set
    : public collie::priv::raw_hash_set<
          collie::priv::NodeHashSetPolicy<T>, Hash, Eq, Alloc>
{
...
// -----------------------------------------------------------------------------
// collie::node_hash_map
// -----------------------------------------------------------------------------
//
// An `collie::node_hash_map<K, V>` is an unordered associative container which
// has been optimized for both speed and memory footprint in most common use
// cases. Its interface is similar to that of `std::unordered_map<K, V>` with
// the following notable differences:
//
...
class node_hash_map
    : public collie::priv::raw_hash_map<
          collie::priv::NodeHashMapPolicy<Key, Value>, Hash, Eq,
          Alloc> 
{
...
// -----------------------------------------------------------------------------
// collie::parallel_flat_hash_set
// -----------------------------------------------------------------------------
template <class T, class Hash, class Eq, class Alloc, size_t N, class Mtx_> // default values in phmap_fwd_decl.h
class parallel_flat_hash_set
    : public collie::priv::parallel_hash_set<
         N, collie::priv::raw_hash_set, Mtx_,
         collie::priv::FlatHashSetPolicy<T>,
         Hash, Eq, Alloc> 
{
...
// -----------------------------------------------------------------------------
// collie::parallel_flat_hash_map - default values in phmap_fwd_decl.h
// -----------------------------------------------------------------------------
template <class K, class V, class Hash, class Eq, class Alloc, size_t N, class Mtx_>
class parallel_flat_hash_map : public collie::priv::parallel_hash_map<
                N, collie::priv::raw_hash_set, Mtx_,
                collie::priv::FlatHashMapPolicy<K, V>,
                Hash, Eq, Alloc> 
{
...
namespace collie {
    ...
    std::size_t erase_if(collie::flat_hash_set<T, Hash, Eq, Alloc>& c, Pred pred) {
        return collie::priv::erase_if(c, std::move(pred));
    }

    template <class T, class Hash, class Eq, class Alloc, class Pred> 
    std::size_t erase_if(collie::node_hash_set<T, Hash, Eq, Alloc>& c, Pred pred) {
        return collie::priv::erase_if(c, std::move(pred));
    }

    template <class T, class Hash, class Eq, class Alloc, size_t N, class Mtx_, class Pred> 
    std::size_t erase_if(collie::parallel_flat_hash_set<T, Hash, Eq, Alloc, N, Mtx_>& c, Pred pred) {
        return collie::priv::erase_if(c, std::move(pred));
    }

    template <class T, class Hash, class Eq, class Alloc, size_t N, class Mtx_, class Pred> 
    std::size_t erase_if(collie::parallel_node_hash_set<T, Hash, Eq, Alloc, N, Mtx_>& c, Pred pred) {
        return collie::priv::erase_if(c, std::move(pred));
    }

    // ======== erase_if for phmap map containers ==================================
    template <class K, class V, class Hash, class Eq, class Alloc, class Pred> 
    std::size_t erase_if(collie::flat_hash_map<K, V, Hash, Eq, Alloc>& c, Pred pred) {
        return collie::priv::erase_if(c, std::move(pred));
    }

    template <class K, class V, class Hash, class Eq, class Alloc, class Pred> 
    std::size_t erase_if(collie::node_hash_map<K, V, Hash, Eq, Alloc>& c, Pred pred) {
        return collie::priv::erase_if(c, std::move(pred));
    }

    template <class K, class V, class Hash, class Eq, class Alloc, size_t N, class Mtx_, class Pred> 
    std::size_t erase_if(collie::parallel_flat_hash_map<K, V, Hash, Eq, Alloc, N, Mtx_>& c, Pred pred) {
        return collie::priv::erase_if(c, std::move(pred));
    }

    template <class K, class V, class Hash, class Eq, class Alloc, size_t N, class Mtx_, class Pred> 
    std::size_t erase_if(collie::parallel_node_hash_map<K, V, Hash, Eq, Alloc, N, Mtx_>& c, Pred pred) {
        return collie::priv::erase_if(c, std::move(pred));
    }
