...
//
//       minimal header providing collie::HashState
//
//       use as:  collie::HashState().combine(0, _first_name, _last_name, _age);
//
...
struct Hash<std::pair<T1, T2>> {
    size_t operator()(std::pair<T1, T2> const& p) const noexcept {
        return collie::HashState().combine(collie::Hash<T1>()(p.first), p.second);
    }
};
...
private:
    ...
    _hash_helper(size_t seed, const TUP &t) const noexcept {
        ...
        seed = Combiner<size_t, sizeof(size_t)>()(seed, collie::Hash<el_type>()(el));
