...
//
// NOTE: `collie::void_t` does not use the standard-specified implementation so
...
struct negation : std::integral_constant<bool, !T::value> {};
  ...
  struct is_trivially_copy_assignable :
     std::integral_constant<bool,
                            ...
                            collie::is_copy_assignable<T>::value> {};
...
// ----------------------------------------------------------------------
    ...
    struct IsHashable<Key,
        collie::enable_if_t<std::is_convertible<
...
private:
    ...
    static std::nullptr_t DoIt() {
        ...
        static_assert(collie::is_copy_assignable<std::hash<Key>>::value,
...
private:
    ...
    template <class P>
    struct ConstantIteratorsImpl<P, collie::void_t<typename P::constant_iterators>>
...
// --------- inline_variable.h
    ...
    #define PHMAP_INTERNAL_EXTERN_DECL(type, name) \
      extern const ::collie::internal::identity_t<type> name;
...
#define PHMAP_INTERNAL_INLINE_CONSTEXPR(type, name, init) \
  ...
  inline constexpr ::collie::internal::identity_t<type> name = init
...
#define PHMAP_INTERNAL_INLINE_CONSTEXPR(var_type, name, init)                  \
  ...
  struct PhmapInternalInlineVariableHolder##name {                             \
    static constexpr ::collie::internal::identity_t<var_type> kInstance = init; \
  };                                                                          \
                                                                              \
  template <class PhmapInternalDummy>                                          \
  constexpr ::collie::internal::identity_t<var_type>                            \
      PhmapInternalInlineVariableHolder##name<PhmapInternalDummy>::kInstance;   \
                                                                              \
  static constexpr const ::collie::internal::identity_t<var_type>&              \
      name = /* NOLINT */                                                     \
...
// working with C++11 variadic templates). `collie::integer_sequence` is designed
...
// `collie::index_sequence` is designed to be a drop-in replacement for C++14's
// `std::index_sequence`.
...
// `collie::optional`, designed to be a drop-in replacement for C++17's
// `std::in_place_t`.
struct in_place_t {};
...
// be specified, such as with `collie::any`, designed to be a drop-in replacement
...
// for C++14's `std::move()`.
template <typename T>
constexpr collie::remove_reference_t<T>&& move(T&& t) noexcept {
  return static_cast<collie::remove_reference_t<T>&&>(t);
}
...
template <typename T>
constexpr T&& forward(
    collie::remove_reference_t<T>& t) noexcept {  // NOLINT(runtime/references)
...
auto apply_helper(Functor&& functor, Tuple&& t, index_sequence<Indexes...>)
    -> decltype(collie::base_internal::Invoke(
        collie::forward<Functor>(functor),
        std::get<Indexes>(collie::forward<Tuple>(t))...)) {
  return collie::base_internal::Invoke(
      collie::forward<Functor>(functor),
      std::get<Indexes>(collie::forward<Tuple>(t))...);
}
...
// pointer. `collie::apply` is designed to be a drop-in replacement for C++17's
...
//       collie::apply(&user_function1, tuple1);
//
//       std::tuple<std::unique_ptr<Foo>> tuple2(collie::make_unique<Foo>());
//       // Invokes the user function that takes ownership of the unique
//       // pointer.
//       collie::apply(&user_function2, std::move(tuple2));
//
//       auto foo = collie::make_unique<Foo>();
//       std::tuple<Foo*, int> tuple3(foo.get(), 42);
//       // Invokes the method Bar on foo with one argument, 42.
//       collie::apply(&Foo::Bar, tuple3);
//
//       std::tuple<int, int> tuple4(8, 9);
//       // Invokes a lambda.
//       collie::apply(user_lambda, tuple4);
//   }
template <typename Functor, typename Tuple>
auto apply(Functor&& functor, Tuple&& t)
    -> decltype(utility_internal::apply_helper(
        collie::forward<Functor>(functor), collie::forward<Tuple>(t),
        collie::make_index_sequence<std::tuple_size<
...
// `obj`.  `collie::exchange` is designed to be a drop-in replacement for C++14's
// `std::exchange`.
//
// Example:
//
//   Foo& operator=(Foo&& other) {
//     ptr1_ = collie::exchange(other.ptr1_, nullptr);
//     int1_ = collie::exchange(other.int1_, -1);
//     return *this;
//   }
template <typename T, typename U = T>
T exchange(T& obj, U&& new_value)
{
    T old_value = collie::move(obj);
    obj = collie::forward<U>(new_value);
    return old_value;
}
...
// Traits to select proper overload and return type for `collie::make_unique<>`.
template <typename T>
...
struct MakeUniqueResult<T[N]> {
    ...
    typename memory_internal::MakeUniqueResult<T>::array make_unique(size_t n) {
        return std::unique_ptr<T>(new typename collie::remove_extent_t<T>[n]());
    }
...
struct allocator_traits 
{
    ...
    // collie::pointer_traits<pointer>::rebind<const value_type>
    using const_pointer =
        memory_internal::ExtractOrT<memory_internal::GetConstPointer, Alloc,
                                    typename collie::pointer_traits<pointer>::
                                    template rebind<const value_type>>;

    // void_pointer:
    // Alloc::void_pointer if present, otherwise
    // collie::pointer_traits<pointer>::rebind<void>
    using void_pointer = memory_internal::ExtractOrT<
        memory_internal::GetVoidPointer, Alloc,
        typename collie::pointer_traits<pointer>::template rebind<void>>;

    // const_void_pointer:
    // Alloc::const_void_pointer if present, otherwise
    // collie::pointer_traits<pointer>::rebind<const void>
    using const_void_pointer = memory_internal::ExtractOrT<
        memory_internal::GetConstVoidPointer, Alloc,
        typename collie::pointer_traits<pointer>::template rebind<const void>>;

    // difference_type:
    // Alloc::difference_type if present, otherwise
    // collie::pointer_traits<pointer>::difference_type
    using difference_type = memory_internal::ExtractOrT<
        memory_internal::GetDifferenceType, Alloc,
        typename collie::pointer_traits<pointer>::difference_type>;
    ...
    // rebind_traits:
    // collie::allocator_traits<rebind_alloc<T>>
    template <typename T>
    using rebind_traits = collie::allocator_traits<rebind_alloc<T>>;
...
protected:
    ...
    constexpr explicit optional_data_dtor_base(in_place_t, Args&&... args)
        : engaged_(true), data_(collie::forward<Args>(args)...) {}
...
protected:
    ...
    constexpr explicit optional_data_base(in_place_t t, Args&&... args)
        : base(t, collie::forward<Args>(args)...) {}
#endif
...
template <typename T,
          bool unused =
          collie::is_trivially_copy_constructible<T>::value &&
          collie::is_trivially_copy_assignable<typename std::remove_cv<T>::type>::value &&
          std::is_trivially_destructible<T>::value>
class optional_data;
...
protected:
    ...
    constexpr explicit optional_data(in_place_t t, Args&&... args)
        : optional_data_base<T>(t, collie::forward<Args>(args)...) {}
#endif
};
...
protected:
    ...
    optional_data(optional_data&& rhs) noexcept(
        collie::default_allocator_is_nothrow::value ||
...
{
    return collie::is_copy_assignable<T>::value &&
                 std::is_copy_constructible<T>::value
             ? copy_traits::copyable
             : collie::is_move_assignable<T>::value &&
...
// Base class for std::hash<collie::optional<T>>:
...
template <typename T>
struct optional_hash_base<T, decltype(std::hash<collie::remove_const_t<T> >()(
                                 std::declval<collie::remove_const_t<T> >()))>
{
    using argument_type = collie::optional<T>;
    using result_type = size_t;
    size_t operator()(const collie::optional<T>& opt) const {
        collie::type_traits_internal::AssertHashEnabled<collie::remove_const_t<T>>();
        if (opt) {
            return std::hash<collie::remove_const_t<T> >()(*opt);
        } else {
...
// -----------------------------------------------------------------------------
// collie::optional class definition
// -----------------------------------------------------------------------------
#if PHMAP_OLD_GCC
...
public:
    ...
    template <typename InPlaceT, typename... Args,
              collie::enable_if_t<collie::conjunction<
                                    std::is_same<InPlaceT, in_place_t>,
                                    std::is_constructible<T, Args&&...> >::value>* = nullptr>
        constexpr explicit optional(InPlaceT, Args&&... args)
        : data_base(in_place_t(), collie::forward<Args>(args)...) {}
    ...
    template <typename U, typename... Args,
        ...
        constexpr explicit optional(in_place_t, std::initializer_list<U> il,
                                    Args&&... args)
        : data_base(in_place_t(), il, collie::forward<Args>(args)...) {
    }
    ...
    template <
        ...
        typename std::enable_if<
            collie::conjunction<collie::negation<std::is_same<
                                                 in_place_t, typename std::decay<U>::type> >,
                              collie::negation<std::is_same<
                                                 optional<T>, typename std::decay<U>::type> >,
                              std::is_convertible<U&&, T>,
                              std::is_constructible<T, U&&> >::value,
            bool>::type = false>
        constexpr optional(U&& v) : data_base(in_place_t(), collie::forward<U>(v)) {}
    ...
    template <
        ...
        typename std::enable_if<
            collie::conjunction<collie::negation<std::is_same<
                              ...
                              collie::negation<std::is_same<
                                                 optional<T>, typename std::decay<U>::type>>,
                              collie::negation<std::is_convertible<U&&, T>>,
                              std::is_constructible<T, U&&>>::value,
            bool>::type = false>
        explicit constexpr optional(U&& v)
        : data_base(in_place_t(), collie::forward<U>(v)) {}

    // Converting copy constructor (implicit)
    template <typename U,
              typename std::enable_if<
                  collie::conjunction<
                      collie::negation<std::is_same<T, U> >,
              ...
              typename std::enable_if<
                  collie::conjunction<
                      ...
                      collie::negation<
                          optional_internal::
                          is_constructible_convertible_from_optional<T, U>>,
                      collie::negation<std::is_convertible<const U&, T>>>::value,
                  bool>::type = false>
    ...
    template <
        ...
        typename std::enable_if<
            collie::conjunction<
                collie::negation<std::is_same<T, U>>, std::is_constructible<T, U&&>,
                collie::negation<
                    optional_internal::is_constructible_convertible_from_optional<
                        T, U>>,
                collie::negation<std::is_convertible<U&&, T>>>::value,
            bool>::type = false>
    ...
    //
    //   struct S { int value; };
    //   optional<S> opt = collie::nullopt;  // Could also use opt = { };
    ...
    template <
        typename U = T,
        typename = typename std::enable_if<collie::conjunction<
                                               collie::negation<
                                                   std::is_same<optional<T>, typename std::decay<U>::type>>,
                                               collie::negation<
                                                   collie::conjunction<std::is_scalar<T>,
    ...
    // optional::reset()
    //
    // Destroys the inner `T` value of an `collie::optional` if one is present.
...
// throw_bad_optional_access() is unreachable.
    ...
    // the `T` sub-object. Throws `collie::bad_optional_access` when the `optional`
    // is empty.
    ...
    constexpr T value_or(U&& v) const& {
        ...
        return static_cast<bool>(*this)
            ? **this
            : static_cast<T>(collie::forward<U>(v));
    }
    template <typename U>
...
// swap()
//
// Performs a swap between two `collie::optional` objects, using standard
// semantics.
//
...
// `collie::optional` can also be explicitly instantiated with
// `make_optional<T>(v)`.
//
...
// Example:
//
//   constexpr collie::optional<int> opt = collie::make_optional(1);
//   static_assert(opt.value() == 1, "");
template <typename T>
constexpr optional<typename std::decay<T>::type> make_optional(T&& v) {
    return optional<typename std::decay<T>::type>(collie::forward<T>(v));
}

template <typename T, typename... Args>
constexpr optional<T> make_optional(Args&&... args) {
    return optional<T>(in_place_t(), collie::forward<Args>(args)...);
}
...
// std::hash specialization for collie::optional.
template <typename T>
struct hash<collie::optional<T> >
    : collie::optional_internal::optional_hash_base<T> {};
...
template <class T>
struct IsTransparent<T, collie::void_t<typename T::is_transparent>>
    : std::true_type {};
...
private:
    collie::optional<allocator_type> alloc_;
    mutable collie::aligned_storage_t<sizeof(slot_type), alignof(slot_type)> slot_space_;
};
...
class node_handle<Policy, PolicyTraits, Alloc,
                  collie::void_t<typename Policy::mapped_type>>
...
template <typename C>
using HasSize =
    std::is_integral<collie::decay_t<decltype(std::declval<C&>().size())>>;
...
using HasData =
    std::is_convertible<collie::decay_t<decltype(GetData(std::declval<C&>()))>*,
                        T* const*>;

// Extracts value type from a Container
template <typename C>
struct ElementType {
  using type = typename collie::remove_reference_t<C>::value_type;
};
...
// `std::vector<T>`, `collie::InlinedVector<T, N>`). All implicit conversions to
// `collie::Span` from such containers will create spans of type `const T`;
// spans which can mutate their values (of type `T`) must use explicit
// constructors.
//
// A `Span<T>` is somewhat analogous to an `collie::string_view`, but for an array
...
//   auto span = collie::Span<const int>(v);
//
//   // Construct a Span explicitly from a C-style array:
//   int a[5] =  {1, 2, 3, 4, 5};
//   auto span = collie::Span<const int>(a);
//
//   // Construct a Span implicitly from a container
//   void MyRoutine(collie::Span<const int> a) {
//     ...
//   }
...
// accept a variety of array types (e.g. a `std::vector`, `collie::InlinedVector`,
...
//   MyRoutine(v);
//
//   collie::InlinedVector<int, 4> my_inline_vector;
//   MyRoutine(my_inline_vector);
//
//   // Explicit constructor from pointer,size
//   int* my_array = new int[10];
//   MyRoutine(collie::Span<const int>(my_array, 10));
template <typename T>
class Span 
{
private:
...
public:
    using value_type = collie::remove_cv_t<T>;
    using pointer = T*;
    ...
    //
    //   void Process(collie::Span<const int> x);
    //   Process({1, 2, 3});
    //
    ...
    //   void Process(collie::Span<const int> ints);
    //
    ...
    //   collie::Span<const int> ints = { 17, 19 };
    //   Process(ints);
    //
    ...
    //   const int foo = 17;
    //   collie::Span<const int> ints = { foo };
    //   Process(ints);
    //
    ...
    //   collie::MakeSpan(vec).subspan(1, 2);  // {11, 12}
    //   collie::MakeSpan(vec).subspan(2, 8);  // {12, 13}
    //   collie::MakeSpan(vec).subspan(1);     // {11, 12, 13}
    //   collie::MakeSpan(vec).subspan(4);     // {}
    //   collie::MakeSpan(vec).subspan(5);     // throws std::out_of_range
    ...
    //
    //   std::vector<int> vec = {10, 11, 12, 13};
    //   collie::MakeSpan(vec).first(1);  // {10}
    //   collie::MakeSpan(vec).first(3);  // {10, 11, 12}
    //   collie::MakeSpan(vec).first(5);  // throws std::out_of_range
    ...
    //
    //   std::vector<int> vec = {10, 11, 12, 13};
    //   collie::MakeSpan(vec).last(1);  // {13}
    //   collie::MakeSpan(vec).last(3);  // {11, 12, 13}
    //   collie::MakeSpan(vec).last(5);  // throws std::out_of_range
...
// Examples:
//
//   void MyRoutine(collie::Span<MyComplicatedType> a) {
//     ...
//   };
...
//   MyRoutine(collie::Span<MyComplicatedType>(my_vector));
//
//   // Use MakeSpan() to make an collie::Span<T>
//   MyRoutine(collie::MakeSpan(my_vector));
//
//   // Construct a span from an array ptr+size
//   collie::Span<T> my_span() {
//     return collie::MakeSpan(&array[0], num_elements_);
//   }
//
...
constexpr auto MakeSpan(C& c) noexcept  // NOLINT(runtime/references)
    -> decltype(collie::MakeSpan(span_internal::GetData(c), c.size())) {
...
// Examples:
//
//   void ProcessInts(collie::Span<const int> some_ints);
//
//   // Call with a pointer and size.
//   int array[3] = { 0, 0, 0 };
//   ProcessInts(collie::MakeConstSpan(&array[0], 3));
//
//   // Call with a [begin, end) pair.
//   ProcessInts(collie::MakeConstSpan(&array[0], &array[3]));
//
//   // Call directly with an array.
//   ProcessInts(collie::MakeConstSpan(array));
//
//   // Call with a contiguous container.
//   std::vector<int> some_ints = ...;
//   ProcessInts(collie::MakeConstSpan(some_ints));
//   ProcessInts(collie::MakeConstSpan(std::vector<int>{ 0, 0, 0 }));
//
template <int&... ExplicitArgumentBarrier, typename T>
...
using Contains = collie::disjunction<std::is_same<T, Ts>...>;

template <class From, class To>
using CopyConst =
    typename std::conditional<std::is_const<From>::value, const To, To>::type;

// Note: We're not qualifying this with collie:: because it doesn't compile under
// MSVC.
template <class T>
...
class LayoutImpl<std::tuple<Elements...>, collie::index_sequence<SizeSeq...>,
                 collie::index_sequence<OffsetSeq...>>
{
private:
    static_assert(sizeof...(Elements) > 0, "At least one field is required");
    static_assert(collie::conjunction<IsLegalElementType<Elements>...>::value,
...
using LayoutType = LayoutImpl<
    std::tuple<Ts...>, collie::make_index_sequence<NumSizes>,
    collie::make_index_sequence<adl_barrier::Min(sizeof...(Ts), NumSizes + 1)>>;
...
public:
    ...
    static_assert(
        collie::conjunction<internal_layout::IsLegalElementType<Ts>...>::value,
        "Invalid element type (see IsLegalElementType)");
    ...
    static constexpr PartialType<sizeof...(Sizes)> Partial(Sizes&&... sizes) {
        ...
        return PartialType<sizeof...(Sizes)>(collie::forward<Sizes>(sizes)...);
    }
...
void* Allocate(Alloc* alloc, size_t n) {
  ...
  using A = typename collie::allocator_traits<Alloc>::template rebind_alloc<M>;
  using AT = typename collie::allocator_traits<Alloc>::template rebind_traits<M>;
  A mem_alloc(*alloc);
...
public:
    ...
    static void construct(Allocator* alloc, slot_type* slot, Args&&... args) {
        ...
        if (kMutableKeys::value) {
            collie::allocator_traits<Allocator>::construct(*alloc, &slot->mutable_value,
                                                         std::forward<Args>(args)...);
        } else {
            collie::allocator_traits<Allocator>::construct(*alloc, &slot->value,
                                                         std::forward<Args>(args)...);
        }
    }
    ...
    static void construct(Allocator* alloc, slot_type* slot, slot_type* other) {
        emplace(slot);
        if (kMutableKeys::value) {
            collie::allocator_traits<Allocator>::construct(
    ...
    static void destroy(Allocator* alloc, slot_type* slot) {
        if (kMutableKeys::value) {
            collie::allocator_traits<Allocator>::destroy(*alloc, &slot->mutable_value);
        } else {
            collie::allocator_traits<Allocator>::destroy(*alloc, &slot->value);
        }
    }
    ...
    static void transfer(Allocator* alloc, slot_type* new_slot,
        ...
        if (kMutableKeys::value) {
            ...
            collie::allocator_traits<Allocator>::construct(*alloc, &new_slot->value,
    ...
    static void swap(Allocator* alloc, slot_type* a, slot_type* b) {
        if (kMutableKeys::value) {
            ...
            collie::allocator_traits<Allocator>::destroy(*alloc, &a->value);
            collie::allocator_traits<Allocator>::construct(*alloc, &a->value,
                                                         std::move(b->value));
            collie::allocator_traits<Allocator>::destroy(*alloc, &b->value);
            collie::allocator_traits<Allocator>::construct(*alloc, &b->value,
                                                         std::move(tmp));
        }
    }
    ...
    static void move(Allocator* alloc, slot_type* src, slot_type* dest) {
        if (kMutableKeys::value) {
            ...
            collie::allocator_traits<Allocator>::destroy(*alloc, &dest->value);
            collie::allocator_traits<Allocator>::construct(*alloc, &dest->value,
                                                          std::move(src->value));
        }
    }
...
// internal locking (like collie::parallel_flat_hash_map).
// -----------------------------------------------------------------------------
class NullMutex {
public:
    NullMutex() {}
...
public:
    ...
    struct DoNothing
    {
        ...
        DoNothing(mutex_type&, collie::adopt_lock_t) noexcept {}
        DoNothing(mutex_type&, collie::defer_lock_t) noexcept {}
        DoNothing(mutex_type&, collie::try_to_lock_t) {}
...
// -----------------------------------
// use as:
//    using Lockable = collie::LockableImpl<mutex_type>;
//    Lockable m;
//  
...
// ---------------------------------------------------------------------------
template <>
class  LockableImpl<collie::NullMutex>: public collie::NullMutex
{
public:
    using mutex_type      = collie::NullMutex;
    using Base            = LockableBaseImpl<collie::NullMutex>;
...
//         use: `collie::AbslMutex` instead of `std::mutex`
// --------------------------------------------------------------------------
    ...
    {
    public:
        using mutex_type      = collie::AbslMutex;
        using Base            = LockableBaseImpl<collie::AbslMutex>;
...
//         use: `collie::srwlock` instead of `std::mutex`
// --------------------------------------------------------------------------
