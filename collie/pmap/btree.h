...
namespace collie {
    ...
    namespace type_traits_internal {
        ...
        class is_trivially_copyable_impl {
            ...
            static constexpr bool kIsCopyOrMoveAssignable =
                collie::is_copy_assignable<ExtentsRemoved>::value ||
                collie::is_move_assignable<ExtentsRemoved>::value;

        public:
            static constexpr bool kValue =
                (collie::is_trivially_copyable<ExtentsRemoved>::value || !kIsCopyOrMoveConstructible) &&
                (collie::is_trivially_copy_assignable<ExtentsRemoved>::value || !kIsCopyOrMoveAssignable) &&
    ...
    namespace swap_internal {
        ...
        // This declaration prevents global `swap` and `collie::swap` overloads from being
        ...
        template <class T>
        struct IsSwappable
            : collie::type_traits_internal::is_detected<IsSwappableImpl, T> {};

        template <class T>
        struct IsNothrowSwappable
            : collie::type_traits_internal::is_detected<IsNothrowSwappableImpl, T> {};

        template <class T, collie::enable_if_t<IsSwappable<T>::value, int> = 0>
...
#undef PHMAP_COMPARE_INLINE_INIT
    ...
    namespace compare_internal {
        ...
        template <typename BoolType,
                  collie::enable_if_t<std::is_same<bool, BoolType>::value, int> = 0>
        constexpr bool compare_result_as_less_than(const BoolType r) { return r; }
        constexpr bool compare_result_as_less_than(const collie::weak_ordering r) {
            return r < 0;
        }
        ...
        template <typename Int,
                  collie::enable_if_t<std::is_same<int, Int>::value, int> = 0>
        constexpr collie::weak_ordering compare_result_as_ordering(const Int c) {
            return c < 0 ? collie::weak_ordering::less
                       : c == 0 ? collie::weak_ordering::equivalent
                       : collie::weak_ordering::greater;
        }
        constexpr collie::weak_ordering compare_result_as_ordering(
            const collie::weak_ordering c) {
            return c;
        }

        template <
            typename Compare, typename K, typename LK,
            collie::enable_if_t<!std::is_same<bool, collie::invoke_result_t<
                                                       Compare, const K &, const LK &>>::value,
                               int> = 0>
            constexpr collie::weak_ordering do_three_way_comparison(const Compare &compare,
                                                                   const K &x, const LK &y) {
            return compare_result_as_ordering(compare(x, y));
        }
        template <
            typename Compare, typename K, typename LK,
            collie::enable_if_t<std::is_same<bool, collie::invoke_result_t<Compare,
            const K &, const LK &>>::value,
                               int> = 0>
            constexpr collie::weak_ordering do_three_way_comparison(const Compare &compare,
                                                                   const K &x, const LK &y) {
            return compare(x, y) ? collie::weak_ordering::less
                : compare(y, x) ? collie::weak_ordering::greater
                : collie::weak_ordering::equivalent;
        }
...
namespace priv {
    ...
    using btree_is_key_compare_to =
        std::is_convertible<collie::invoke_result_t<Compare, const T &, const T &>,
                            collie::weak_ordering>;
    ...
    struct StringBtreeDefaultLess {
        ...
        StringBtreeDefaultLess(collie::Less<std::string_view>) {}  // NOLINT

        collie::weak_ordering operator()(const std::string_view &lhs,
                                        const std::string_view &rhs) const {
            return compare_internal::compare_result_as_ordering(lhs.compare(rhs));
        }
#else
        collie::weak_ordering operator()(const std::string &lhs,
    ...
    struct StringBtreeDefaultGreater {
        ...
        collie::weak_ordering operator()(std::string_view lhs,
    ...
    template <>
    struct key_compare_to_adapter<collie::Less<std::string>> {
        using type = StringBtreeDefaultLess;
    };
    ...
    template <>
    struct key_compare_to_adapter<collie::Less<std::string_view>> {
        using type = StringBtreeDefaultLess;
    };
    ...
    struct common_params {
        ...
        using node_count_type =
            collie::conditional_t<(kNodeSlotSpace / sizeof(slot_type) >
    ...
    struct map_params : common_params<Key, Compare, Alloc, TargetNodeSize, Multi,
                                      collie::priv::map_slot_policy<Key, Data>> {
    ...
    struct set_slot_policy {
        ...
        static void construct(Alloc *alloc, slot_type *slot, Args &&... args) {
            collie::allocator_traits<Alloc>::construct(*alloc, slot,
                                                       std::forward<Args>(args)...);
        }

        template <typename Alloc>
        static void construct(Alloc *alloc, slot_type *slot, slot_type *other) {
            collie::allocator_traits<Alloc>::construct(*alloc, slot, std::move(*other));
        }

        template <typename Alloc>
        static void destroy(Alloc *alloc, slot_type *slot) {
            collie::allocator_traits<Alloc>::destroy(*alloc, slot);
        }
    ...
    struct upper_bound_adapter {
        ...
        bool operator()(const K &a, const LK &b) const {
            ...
            return !collie::compare_internal::compare_result_as_less_than(comp(b, a));
        }
    ...
    public:
        ...
        using use_linear_search = std::integral_constant<
            ...
            (std::is_same<collie::Less<key_type>, key_compare>::value ||
    ...
    private:
        using layout_type = collie::priv::Layout<btree_node *, field_type,
...
#pragma GCC diagnostic ignored "-Warray-bounds"
        ...
        void clear_child(size_type i) {
            collie::priv::SanitizerPoisonObject(&mutable_child(i));
        }
        void set_child(size_type i, btree_node *c) {
            collie::priv::SanitizerUnpoisonObject(&mutable_child(i));
            mutable_child(i) = c;
...
#pragma GCC diagnostic pop
        ...
        SearchResult<int, true> linear_search_impl(
            ...
            while (s < e) {
                const collie::weak_ordering c = comp(key(s), k);
                if (c == 0) {
        ...
        SearchResult<int, true> binary_search_impl(
            ...
            if (is_multi_container::value) {
                ...
                while (s != e) {
                    const int mid = (s + e) >> 1;
                    const collie::weak_ordering c = comp(key(mid), k);
                    if (c < 0) {
                        s = mid + 1;
                    } else {
                        e = mid;
        ...
        static btree_node *init_leaf(btree_node *n, btree_node *parent,
            ...
            collie::priv::SanitizerPoisonMemoryRegion(
    ...
    private:
        ...
        void value_init(const size_type i, allocator_type *alloc, Args &&... args) {
            collie::priv::SanitizerUnpoisonObject(slot(i));
            params_type::construct(alloc, slot(i), std::forward<Args>(args)...);
        }
        void value_destroy(const size_type i, allocator_type *alloc) {
            params_type::destroy(alloc, slot(i));
            collie::priv::SanitizerPoisonObject(slot(i));
        }
        ...
        void uninitialized_move_n(const size_type n, const size_type i,
                                  ...
                                  allocator_type *alloc) {
            collie::priv::SanitizerUnpoisonMemoryRegion(
    ...
    public:
        ...
        btree(btree &&x) noexcept
            ...
            rightmost_(collie::exchange(x.rightmost_, EmptyNode())),
            size_(collie::exchange(x.size_, 0)) {
    ...
    private:
        ...
        node_type *allocate(const size_type sz) {
            return reinterpret_cast<node_type *>(
                collie::priv::Allocate<node_type::Alignment()>(
        ...
        void deallocate(const size_type sz, node_type *node) {
            collie::priv::Deallocate<node_type::Alignment()>(
    ...
    constexpr bool btree<P>::static_assert_validation() {
        ...
        // Verify that key_compare returns an collie::{weak,strong}_ordering or bool.
        using compare_result_type =
            collie::invoke_result_t<key_compare, key_type, key_type>;
        static_assert(
            std::is_same<compare_result_type, bool>::value ||
            std::is_convertible<compare_result_type, collie::weak_ordering>::value,
            "key comparison function must return collie::{weak,strong}_ordering or "
            "bool.");
    ...
    public:
        ...
        mapped_type &at(const key_arg<K> &key) {
            auto it = this->find(key);
            if (it == this->end())
                base_internal::ThrowStdOutOfRange("collie::btree_map::at");
            return it->second;
        }
        template <typename K = key_type>
    ...
    // Swaps the contents of two `collie::btree_set` containers.
    // -------------------------------------------------------
    ...
    // Swaps the contents of two `collie::btree_multiset` containers.
    // ------------------------------------------------------------
    template <typename K, typename C, typename A>
    ...
    // Swaps the contents of two `collie::btree_map` containers.
    // -------------------------------------------------------
    ...
    // Swaps the contents of two `collie::btree_multimap` containers.
    // ------------------------------------------------------------
    template <typename K, typename V, typename C, typename A>
